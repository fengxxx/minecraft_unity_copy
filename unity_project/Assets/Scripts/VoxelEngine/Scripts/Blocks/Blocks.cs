...
public class Blocks
{
	...
	/// </summary>
    public static Dictionary<int, Block> blocks;
	...
	public static int AddTexture(Texture2D texture)
	{
        ...
        {
            if (texture.name==textures[id].name)
            {
                return id;
            }
        }
	...
	public static void BuildTextureAtlas()
	{
        ...
        // Save uvs
        //Debug.Log(textures.Keys);
		textureCoordinates = new Dictionary<int, Vector2[]> ();
		foreach (KeyValuePair<int, Texture2D> texture in textures)
		{
            //Debug.Log(textures[texture.Key].name);
	...
	{
		//Debug.Log(textureId);
        //Debug.Log(textureCoordinates.Count());
	...
	public static Block RegisterBlock(int blockId)
	{
		...
		{
			Debug.Log("Block with id " + blockId + " already exists!");
			return null;
		}
	...
	{
		//Debug.Log(blocks);
		//Debug.Log(blocks.ContainsKey(blockId));
		//Debug.Log(blockId);
